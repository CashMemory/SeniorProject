...
import math, json, time, sys, cv2, argparse, copy, torch, torch2trt
...
'''
Initializes the camera with a width and height in pixels
Returns: Output video object for writing video to disk
'''
def SetupCamera(videoName, deviceId=0, width=640, height=480):
    ...
    fourcc = cv2.VideoWriter_fourcc('m', 'p', '4', 'v')                                         #Save as .mp4
    outVideo = cv2.VideoWriter(videoName, fourcc, cap.get(cv2.CAP_PROP_FPS), (width, height))

    return outVideo

    
def ReleaseVideo(camera, videoOutput):
